...
@section('title', __('messages.edit_image'))

@section('page-content')
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('messages.edit_image') }}</h1>
        <a href="{{ route('admin.slideshow-images.index') }}" class="text-sm text-primary-600 hover:text-primary-800">{{ __('messages.back') }}</a>
    </div>

    <div class="mt-6 max-w-lg">
        <form method="POST" action="{{ route('admin.slideshow-images.update', $slideshowImage) }}" class="bg-white rounded-lg shadow p-6 space-y-4">
            @csrf @method('PUT')

            <div>
                <label for="image_path" class="block text-sm font-medium text-gray-700">{{ __('messages.image_path') }}</label>
                <input type="text" name="image_path" id="image_path" value="{{ old('image_path', $slideshowImage->image_path) }}" required
            ...
            <div>
                ...
                <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $slideshowImage->sort_order) }}" min="0"
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_active" id="is_active" value="1"
                       {{ $slideshowImage->is_active ? 'checked' : '' }}
                       class="rounded border-gray-300 text-primary-600 focus:ring-primary-500">
                <label for="is_active" class="text-sm font-medium text-gray-700">{{ __('messages.active') }}</label>
            </div>

            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="px-4 py-2 bg-primary-600 text-white text-sm font-medium rounded-md hover:bg-primary-700 cursor-pointer">{{ __('messages.update') }}</button>
