...
@section('page-content')
    ...
    @php $uncategorized = $course->uncategorizedContents(); @endphp
                    ...
                    @if($kind === 'media' && $item->type === 'mp3')
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                            ...
                            <div class="flex items-center gap-1 shrink-0">
                                <form method="POST" action="{{ route('admin.media.move-up', $item) }}" class="inline">
                                    @csrf
                                ...
                                <form method="POST" action="{{ route('admin.media.move-down', $item) }}" class="inline">
                                    @csrf
                                ...
                                <form method="POST" action="{{ route('admin.media.destroy', $item) }}" class="inline">
                                    @csrf @method('DELETE')
                                    <button type="button" @click.prevent="$dispatch('confirm-open', { action: '{{ route('admin.media.destroy', $item) }}', method: 'DELETE', message: '{{ __('messages.confirm_delete_file') }}' })" class="text-sm text-red-600 hover:text-red-800 cursor-pointer">{{ __('messages.delete') }}</button>
                                </form>
                            </div>
                        </div>
                    @elseif($kind === 'media')
                        ...
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <a href="{{ $item->url }}" target="_blank" class="text-xs text-primary-600 hover:text-primary-900">{{ __('messages.watch_on_youtube') }}</a>
                                </div>
                            </div>
                            <div class="flex items-center gap-1 shrink-0">
                                <form method="POST" action="{{ route('admin.youtube-videos.move-up', $item) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="p-0.5 text-gray-400 hover:text-gray-600 cursor-pointer" title="{{ __('messages.move_up') }}">
                                        <svg class="w-3.5 h-3.5 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg>
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('admin.youtube-videos.move-down', $item) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="p-0.5 text-gray-400 hover:text-gray-600 cursor-pointer" title="{{ __('messages.move_down') }}">
                                        <svg class="w-3.5 h-3.5 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('admin.youtube-videos.destroy', $item) }}" class="inline">
                                    @csrf @method('DELETE')
                                    <button type="button" @click.prevent="$dispatch('confirm-open', { action: '{{ route('admin.youtube-videos.destroy', $item) }}', method: 'DELETE', message: '{{ __('messages.confirm_delete') }}' })" class="text-sm text-red-600 hover:text-red-800 cursor-pointer">{{ __('messages.delete') }}</button>
                                </form>
                            </div>
                        </div>
                    @endif
