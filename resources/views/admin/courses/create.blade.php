...
@section('page-content')
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-4">
        ...
        <span>/</span>
        <span class="text-gray-900">{{ __('messages.create') }}</span>
    </div>

    <h1 class="text-2xl font-bold text-gray-900">{{ __('messages.create_course') }}</h1>

    <div class="mt-6 max-w-2xl">
        <form method="POST" action="{{ route('admin.courses.store') }}" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-6 space-y-6" x-data="{ slug: '{{ old('slug') }}' }">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" required
                       ...
                       x-on:input="slug = $el.value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '')">
            ...
            <div>
                ...
                <div id="editor" class="mt-1" style="min-height: 250px;"></div>
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="checkbox" name="show_on_homepage" id="show_on_homepage" value="1" {{ old('show_on_homepage', true) ? 'checked' : '' }}
                       class="rounded border-gray-300 text-primary-600 focus:ring-primary-500">
