    ...
    @elseif($kind === 'media')
        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
            <div class="flex items-center gap-3 w-full">
                ...
                @auth
                    <a href="{{ route('media.download', $item) }}" class="shrink-0 px-3 py-1.5 text-xs font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700">{{ __('messages.download') }}</a>
                @else
                    <a href="{{ route('login') }}" class="shrink-0 px-3 py-1.5 text-xs font-medium text-primary-600 border border-primary-300 rounded-md hover:bg-primary-50">{{ __('messages.sign_in_to_view') }}</a>
                @endauth
            </div>
